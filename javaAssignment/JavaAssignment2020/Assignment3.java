...
public class Assignment3 {
    ...
    public static void ping(String host, int no_of_times) {
        String command = "ping -c " + no_of_times + " " + host;
        String command_output = "";
        int counter = 0;
        int index = 0;
        String time = "";
        ArrayList<Float> timeTaken = new ArrayList<Float>();
        try {
            ...
            while (counter < no_of_times) {
                ...
                timeTaken.add(Float.parseFloat(time));
                command_output = stdInput.readLine();
                counter++;
            ...
            if (timeTaken.size() % 2 != 0)
                System.out.println("Median of time taken to ping " + timeTaken.get(timeTaken.size() / 2));
            else
                System.out.println("Median of time taken to ping " + (timeTaken.get(timeTaken.size() / 2) + timeTaken.get((timeTaken.size() / 2) - 1)) / 2);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
